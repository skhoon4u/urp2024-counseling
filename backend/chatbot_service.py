...
from flask import Flask, request, jsonify #웹서버 프레임워크
from flask_cors import CORS #CORS(Cross-Origin Resource Sharing)위한 라이브러리
...
save_dir = './' # 모델 저장 디렉토리 설정

def load_trained_model(checkpoint_dir, from_tf=False): #모델 로드 함수 정의
    try:
...
def chat():
    try:
        data = request.get_json() #POST 요청으로 전달된 JSON 데이터를 가져옴
        ...
        if 'conversation_history' not in request.cookies:
            ...
            conversation_history = request.cookies.get('conversation_history').split('|') #대화 내역을 쿠키에서 가져옴
        ...
        input_text = ' '.join(conversation_history_with_eos) #대화 내역을 하나의 문자열로 결합
